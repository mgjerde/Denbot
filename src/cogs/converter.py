...
units = {
    ...
    "Inches (in)": {
        "Meters (m)": 0.0254,
        "Feet (ft)": 0.0833333333,
        "Yard (yd)": 0.0277777778,
        "Mile (mi)": 0.000015782828,
    },
    "Yard (yd)": {
        "Meters (m)": 0.9144,
        "Feet (ft)": 3,
        "Inches (in)": 36,
        "Mile (mi)": 0.000568181818,
    },
...
class Converter(commands.Cog):
    ...
    async def convert_command(
        ...
        embed = discord.Embed(
            title=f"{amount} {unit} is:", color=discord.Color.random()
        )
        for convertedunit in units[unit].keys():
            if callable(units[unit][convertedunit]):
                embed.add_field(
                    name=f"{convertedunit}",
                    value=units[unit][convertedunit](amount),
                    inline=True,
                )
            else:
                embed.add_field(
                    name=f"{convertedunit}",
                    value=round(units[unit][convertedunit] * amount, 3),
                    inline=True,
                )
