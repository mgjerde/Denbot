...
class Winter(commands.Cog):
    ...
    async def snowball(
        ...
        if throw == 0:
            targets = [
                "It barely grazes the shoulder.",
                "It hits straight in the face! ",
                "It hits the kneecap.",
                "It splatters all over the chest.",
            ]
        ...
        snowball.set_footer(
            text=f"{ctx.author.name} throws a snowball at {member.name}. {result}",
            icon_url="https://icons.iconarchive.com/icons/arrioch/christmas/128/snowball-icon.png",
        )
